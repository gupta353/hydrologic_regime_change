...
Note: Only the watersheds for which streamflow data is avaialble between 1980 and 2013 is copied
...
for basin in listFile:
    if (basin not in list_complete_watersheds_0):
...
for fname in remaining_basins:
   #   fname = '01047000_GLEAMS_CAMELS_data.txt'
    ...
    if (len_missing_ind == 0 and len_nan_ind == 0):
        destination = direc + '/CAMELS_GLEAMS_combined_data/complete_watersheds_1/' + fname
        shutil.copy(filename,direc + '/CAMELS_GLEAMS_combined_data/complete_watersheds_1')