...
train_datenum_1 = datetime.date(1999,10,1).toordinal()
train_datenum_2 = datetime.date(2008,9,30).toordinal()
val_datenum_1 = datetime.date(1986,10,1).toordinal()
val_datenum_2 = datetime.date(1989,9,30).toordinal()
...
for seed in seeds:
    ...
    # save model
    fname = 'model_saved_normalized_' + str(seed) + '.pth'
    ...
    sname = 'basin_NSE_normalized_' + str(seed) + '.txt'
...
obs_pred_avg  = []
for basin_ind in range(len(basins_used)):
    ypred = 0
...
# save NSE values to a textfile
sname = 'basin_NSE_normalized_' + 'avg' + '.txt'
save_direc = 'D:/Research/non_staitionarity/codes/results/LSTM_global/' + save_subdir
filename  = save_direc + '/' + sname
fid = open(filename, 'w')
fid.write('BASIN\tNSE\tsd\n')
...
# save NSE BS values
fname = 'nse_uncertainty_normalized'
direc = 'D:/Research/non_staitionarity/codes/results/LSTM_global/' + save_subdir
filename = direc + '/' + fname
