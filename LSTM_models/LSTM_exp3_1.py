...
Similirity between watersheds is identified using the watershed static attributes only: long-term climate such as aridty, soil properties, geological properties, 
forest fraction

Author: Abhinav Gupta (Created: 25 July 2022)
...
clus_mean = np.mean(static_stat_ts[basin_used_label_ind,:], axis = 0)

# find the distance between test cluster mean and training data points 
dist = []
for ind in range(static_stat_tr.shape[0]):
    dist.append(Euclid(static_stat_tr[ind,:], clus_mean))
dist = np.array(dist)
