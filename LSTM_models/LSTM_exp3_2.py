...
Similirity between watersheds is identified using the both climatic statistics (as used in experiemnt 3_0) and watershed static attributes (as used in experiemnt 3_1)
...
for train in train_data:
    ...
    stat = train[0][7:-1]       # list of static attributes
    static_stat_tr.append(stat)

cl_stat_tr = np.array(cl_stat_tr)
static_stat_tr = np.array(static_stat_tr)
...
for test in test_data_A:
    ...
    stat = test[0][7:-1]       # list of static attributes
    static_stat_ts.append(stat)
cl_stat_ts = np.array(cl_stat_ts)
static_stat_ts = np.array(static_stat_ts)
...
static_m = np.mean(np.concatenate((static_stat_tr, static_stat_ts)), axis = 0)
static_std = np.std(np.concatenate((static_stat_tr, static_stat_ts)), axis = 0)
...
for col in range(static_stat_tr.shape[1]):
    static_stat_tr[:,col] = (static_stat_tr[:,col] - static_m[col])/static_std[col]
    static_stat_ts[:,col] = (static_stat_ts[:,col] - static_m[col])/static_std[col]
...
cl_clus_mean = np.mean(cl_stat_ts[basin_used_label_ind,:], axis = 0)

# choose cluster center at test basins in static attribute space 
static_clus_mean = np.mean(static_stat_ts[basin_used_label_ind,:], axis = 0)

# find the distance between test cluster mean and training data points in climatic statistics space 
cl_dist = []
for ind in range(cl_stat_tr.shape[0]):
    cl_dist.append(Euclid(cl_stat_tr[ind,:], cl_clus_mean))
cl_dist = np.array(cl_dist)

# find the distance between test cluster mean and training data points in static attribute space 
static_dist = []
for ind in range(static_stat_tr.shape[0]):
    static_dist.append(Euclid(static_stat_tr[ind,:], static_clus_mean))
static_dist = np.array(static_dist)

# combine distance coputed in climatic statistics and static atribute space
num_cl = len(cl_clus_mean)      # number of dimension in climatic space
num_static = len(static_clus_mean)      # number of dimension in static attribute space
ratio = num_cl/num_static
dist = (cl_dist**2 + ratio**2*static_dist**2)**0.5
