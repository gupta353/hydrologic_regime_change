...
d = 0.35
arparams = np.array([0.25,0.12,0.1])    # autoregressive coefficients
maparams = np.array([0.90,0.85,0.20])        # moving-average coefficients
ar = np.r_[1, -arparams]
ma = np.r_[1, maparams]
...
plotFARIMA.plotResidual(model_fit.resid)

# extract confidence intervals
conf = model_fit.conf_int(0.05)
print(conf)

theta, conf_hessian1, conf_module, conf1 = FarimaModule.ParEstIterd(data,p,q,one_sided,pt,f,pxx_den)

# Compute Hessian matrix of -loglikeli (Based on Whittle's approximate likelihood) and estimate the 95% confidence interval
one_sided = True
